...
- 单个源文件默认不能超过 400 行；超过时必须优先按高内聚低耦合拆分模块，不能用大文件继续堆职责。注意：lint 当前未强制该规则（`apps/zcode-cli/.oxlintrc.json` 关闭了 max-lines，避免根仓库配置经向上查找意外套用到 CLI；存量千行级文件按模块拆分是 P6+ 债务），新代码仍应遵守本条。
